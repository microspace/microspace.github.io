...
var Pegman = {
    ...
    reset2: function () {
        ...
        this.pegmanSprite.scale.x = scalefactor;
        this.pegmanSprite.scale.y = scalefactor;
        try {
            sinkflag = false;
            hitflag = false;
            b.visible = true;
    ...
    playNextAction: function () {
        ...
        switch (action) {
            ...
            this.direction = Maze.DirectionType.EAST;
                ...
                this.moveNSWE(this.posX + Maze.SQUARE_SIZE * scalefactor * step[0] * stepcount, this.posY + Maze.SQUARE_SIZE * scalefactor * step[1] * stepcount, stepcount);
                break;
            case "uturn":
                if (weapon.fireAngle == Phaser.ANGLE_RIGHT) {
                    this.direction = Maze.DirectionType.WEST;
                    var step = Maze.getStepInDirection["WEST"];
                    this.pegmanSprite.scale.x = -1 * this.pegmanSprite.scale.x;
...
Pegman.moveNSWE = function (x, y, stepcount = 1) {
    ...
    console.log(this.posX, this.posY);
    this.tween.to({
        x: this.posX * scalefactor,
        y: this.posY * scalefactor,
    }, this.animSpeedByStep * stepcount, Phaser.Easing.Linear.In);
    this.tween.onComplete.addOnce(function () {
        this.pegmanSprite.animations.play("STAND");
        this.dposX = Math.floor(x / 64);
        this.dposY = Math.floor(y / 64);
        //TopDownGame.game.camera.follow(this.pegmanSprite, Phaser.Camera.FOLLOW_LOCKON, 0.1, 0.1);
        this.playNextAction();
    }, this);
    this.tween.start();
},
